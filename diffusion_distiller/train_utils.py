...
from diffusion_distiller.moving_average import moving_average
from diffusion_distiller.strategies import *
...
def make_visualization_(diffusion, device, image_size, need_tqdm=False, eta=0, clip_value=1.2):
    ...
    imgs = p_sample_loop(diffusion, noise, extra_args, "cuda", samples_to_capture=1, need_tqdm=need_tqdm, eta=eta, clip_value=clip_value)
    images_ = []
    for images in imgs:
...
def make_visualization_withprior(diffusion, device, image_size, prior, need_tqdm=False, eta=0, clip_value=1.2):
    extra_args = {}
    tmp = []
    ...
    imgs = p_sample_loop(diffusion, tmp, extra_args, "cuda", samples_to_capture=1, need_tqdm=need_tqdm, eta=eta, clip_value=clip_value)
    images_ = []
    # print(imgs[0].shape)
    # for images in imgs:
    #     images = images.split(1, dim=0)
...
def make_visualization(diffusion, device, image_size, need_tqdm=False, eta=0, clip_value=1.2):
    ...
    images_ = (255 * images_).clip(0, 255).astype(np.uint8)
    return images_
...
class DiffusionTrain:
    ...
    def train(self, train_loader, diffusion, model_ema, model_lr, device, make_extra_args=make_none_args, on_iter=default_iter_callback):
        ...
        for img in pbar:
            ...
            extra_args = make_extra_args(img, 0, device)
