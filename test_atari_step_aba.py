...
def mini_test(env, model, diffusion, config, dtype, max_frames_per_episode=1000, seed = 1000, steps = 1):
    ...
    for frame_idx in range(1, max_frames_per_episode + 1):
        ...
        #diffusion
        att_state_diff = diffusion.p_sample_loop_with_prior((att_state.transpose(0,2,1)), samples = 1, steps = steps).cpu().numpy().transpose(0,1,3,2)
        ...
        if done:
            ...
            this_episode_frame += 1
    fps = frame_idx/(time.time()-begin_time)
...
def main(args):
    ...
    # random.seed(seed)
    # torch.manual_seed(seed)
    for steps in [1,5,10,15,20,25,30]:
        seed = 5004
        #seed = 1000
        reward  = []
        fps = []
        for i in range(5):
            tmp, tmp1 = mini_test(env, model, diffusion, config, dtype, max_frames_per_episode=5000, seed = seed+i, steps = steps)
            print(steps, seed+i, tmp)
            reward.append(tmp)
            fps.append(tmp1)
        print("average fps is ", np.mean(fps))
        file = open("pong-vanila-aba-steps-"+str(steps)+"fps.csv", 'w')
