...
def make_atari_cart(env_id):
    env = gym.make(env_id)
    return env

def wrap_deepmind(env, episode_life=True, clip_rewards=True, frame_stack=False, scale=False, color_image=False, central_crop=False, crop_shift=0, restrict_actions=0):
    """Configure environment for DeepMind-style Atari.
    """
    print('using episode life:', episode_life)
    if episode_life:
    ...
    env = WarpFrame(env, color_image=color_image, central_crop=central_crop, crop_shift=crop_shift)
    if scale:
