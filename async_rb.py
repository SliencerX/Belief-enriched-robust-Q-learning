...
def worker_add_belief(args):
    import time
    global replay_buffer
    global numpy_buffers
    action, reward, belief, done = args
    # states already copied to shared memory, avoid IPC
    _, _, state, next_state, _, _ = numpy_buffers
    replay_buffer.add(obs=state, act=action, rew=reward, belief = belief, next_obs=next_state, done=done)
    return None
...
class AsyncReplayBuffer_withbelief(object):
    def __init__(self, initial_state=None, batch_size=None):
        self.total_items = 0
        self.synchronous = False
        if self.synchronous:
    ...
    def add(self, obs, act, rew, belief, next_obs, done):
        self.async_add(obs, act, rew, belief, next_obs, done)

    def async_add(self, obs, act, rew, belief, next_obs, done):
        self.total_items += 1
        if self.synchronous:
            """
            if self.add_args is not None:
            ...
            self.b_state[:] = obs
            self.b_next_state[:] = next_obs
            self.add_res = self.pool.apply_async(worker_add_belief, [(act, rew, belief, done)])
            return
    ...
    def async_sample(self, batch_size, buffer_beta):
        if self.synchronous:
            ...
            self.sample_res = self.pool.apply_async(worker_sample_belief, [(batch_size, buffer_beta)])
        return
