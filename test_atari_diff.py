...
def mini_test(env, model, diffusion, config, dtype, max_frames_per_episode=1000, seed = 1000):
    ...
    for frame_idx in range(1, max_frames_per_episode + 1):
        ...
        #diffusion
        att_state_diff = diffusion.p_sample_loop_with_prior((att_state.transpose(0,2,1)), samples = 1, steps = 25).cpu().numpy().transpose(0,1,3,2)
        ...
        restore_img_1 = diffusion.p_sample_loop_with_prior((state.transpose(0,2,1)/255), samples = 1, steps = 25).cpu().numpy().transpose(0,1,3,2)
        ...
        ori_diff.append(np.linalg.norm(att_state - state/255))
        purifiy_diff.append(np.linalg.norm(att_state_diff[0] - restore_img_1[0]))
        ...
        if done:
            ...
            this_episode_frame += 1
    return episode_reward, ori_diff, purifiy_diff
...
def main(args):
    ...
    diffusion.load_state_dict(torch.load("./results_pong/model-150.pt")['model'])
    ...
    #model_path = "sa-dqn_models/models/Freeway-natural.model"
    ...
    reward  = []
    for i in range(1):
        tmp, ori_diff, purifiy_diff = mini_test(env, model, diffusion, config, dtype, max_frames_per_episode=5000, seed = seed+i)
        print(seed+i, tmp)
        reward.append(tmp)
    print("average reward is ", np.mean(reward))
    file = open("pgd-15-255-pong-ori_diff.csv", 'w')
    writer = csv.writer(file)
    writer.writerow(ori_diff)
    file.close()
    file = open("pgd-15-255-pong-purifiy_diff.csv", 'w')
    writer = csv.writer(file)
    writer.writerow(purifiy_diff)
    file.close()
