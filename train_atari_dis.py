...
from async_rb import AsyncReplayBuffer, AsyncReplayBuffer_withbelief
from gridworld import *
from ibp import *
from data_utils import *
from pfrnns.model import Belief_model
from pfrnns.evaluate import parse_args,get_checkpoint
...
#     encoder = torch.load("autoencoder")
...
def get_belief(model, args, env_map, obs, pos, actions):
    ...
    with torch.no_grad():
        ...
        loss, log_loss, particle_pred = model.step(
            obs, actions, pos, args)
        temp = particle_pred.squeeze(1)
        last = temp[:,-1,:]
        result = []
        result.append(np.asarray(last.detach().cpu()))
...
def get_action_withbelief(env, model, belief, eps):
    value = dict()
    tensor_states = torch.from_numpy(belief).cuda().to(torch.float32)
    q_values = model.forward(tensor_states)
    for action in range(env.action_space.n):
        ...
        #print(temp)
        value[action]=np.min(q_values[:,action].data.cpu().numpy())
...
def get_action_withbelief_atari(env, model, autoencoder, belief, eps):
    value = dict()
    for action in range(env.action_space.n):
        temp = []
        tensor_states = []
        # for b in belief:
        #     state_tensor = torch.tensor(np.asarray(b)).cuda()
        #     ori_state_tensor = autoencoder.decoder(state_tensor)
        #     tensor_states.append(ori_state_tensor)
        # tensor_states = torch.stack(tensor_states).cuda().to(torch.float32)
        tensor_states = autoencoder.decoder(torch.from_numpy(belief).cuda().to(torch.float32))
        #print(tensor_states)
        #tensor_state = torch.from_numpy(np.ascontiguousarray(b)).unsqueeze(0).cuda().to(torch.float32)
        #print(np.min(model.forward(tensor_states)[:,action].data.cpu().numpy()))
        #temp.append(model.forward(tensor_states)[0][action].data.cpu())
        #print(temp)
        advantage = model.features.advantage(tensor_states)
        values = model.features.value(tensor_states)
        q_value = values + advantage - torch.sum(advantage, dim=1, keepdim=True) / model.num_actions
        value[action]=np.min(q_value[:,action].detach().cpu().numpy())
    action = (max(value, key = value.get))
    mask = np.random.choice(np.arange(0, 2), p=[1-eps, eps])
    action = (1-mask) * action + mask * np.random.randint(env.action_space.n)
    return action
...
def get_att_state(model, true_state, max_diff_norm = 1/255, sample_time = 10, model_type = "ours"):
    possible_action = set()
    ...
    #     x = np.random.uniform(-max_diff_norm, max_diff_norm, size = (1,84,84))
    ...
    purturb_states = np.clip((np.random.uniform(-max_diff_norm, max_diff_norm, size = (sample_time,1,84,84)) + true_state), 0 ,1)
    tensor_purturb_states = torch.from_numpy(np.ascontiguousarray(purturb_states)).unsqueeze(1).cuda().to(torch.float32)

    #tensor_purturb_states = torch.stack(tensor_purturb_states)
    #print(tensor_purturb_states.shape)
    if model_type == "ours":
        actions = get_defender_policy_batch(model, tensor_purturb_states)
    else:
        actions = []
        for i in tensor_purturb_states:
            actions.append(model.act(i, 0)[0])
    index = 0
    for action, purturb_state in zip(actions,purturb_states):
        ...
        values[index] = model.forward(tensor_true_state)[0][action].data.cpu()
        if len(possible_action) == model.env.action_space.n:
            break
        index +=1
    #print(len(possible_action))
    att_state_index = min(values, key = values.get)
    return purturb_states[att_state_index]

def get_defender_policy(model, X, max_diff_norm = 0.1, max_iter= 5):
    values = dict()
...
def get_defender_policy_batch(model, X, max_diff_norm = 1/255, max_iter = 5):
    #print(model)
    X = X.squeeze(1)
    loss_fn = torch.nn.CrossEntropyLoss()
    values_list = []
    ...
    for action in range(model.env.action_space.n):
        ...
        for i in range(max_iter):
            ...
            # adjust to be within [-epsilon, epsilon]
            eta = torch.clamp(X_adv.data - X.data, -max_diff_norm, max_diff_norm)
            X_adv.data = torch.clamp(X.data + eta, 0 ,1)
        count = 0
        for adv in X_adv:
...
def find_maxmin_with_belief(model, next_states, belief):
    ...
    for next_state, possible_states in zip(next_states, belief):
        ...
        for action in possible_action:
            value[action] = model.forward(next_state)[0][action].data.cpu()
        result.append(np.min(list(value.values())))
    result = np.asarray(result)
    result_tensor = torch.from_numpy(result).cuda()
    return result_tensor

def find_maxmin_with_belief_atari(model, autoencoder, next_states, belief):
    result = []
    for next_state, possible_states in zip(next_states, belief):
        possible_action = set()
        value = dict()
        ...
        #     ori_state_tensor = autoencoder.decoder(state_tensor)
        #     tensor_states.append(ori_state_tensor)
        tensor_states = autoencoder.decoder(torch.from_numpy(possible_states).cuda())
        #tensor_states = torch.stack(tensor_states)
        #print(model.forward(tensor_states))
        advantage = model.features.advantage(tensor_states)
        values = model.features.value(tensor_states)
        q_value = values + advantage - torch.sum(advantage, dim=1, keepdim=True) / model.num_actions
        actions = q_value.max(1)[1].data.cpu().numpy()
        for action in actions:
            #print(actions)
            #action = model.act(state_tensor, 0)[0].cpu().numpy()
            possible_action.add(int(action))
...
def find_maxmin(model, next_states, max_diff_norm = 1/255, max_iter = 10, sample_time = 100):
    d = model.env.observation_space.shape[0]
    result = []
...
def compute_td_loss(current_model, target_model, batch_size, replay_buffer, per, use_cpp_buffer, use_async_rb, optimizer, gamma, memory_mgr, robust, our_alg = False, **kwargs):
    ...
    if our_alg:
        tgt_next_q_value = find_maxmin_with_belief(target_model, next_state, belief)
        #tgt_next_q_value = find_maxmin_with_belief_atari(target_model, encoder, next_state, belief)
...
def main(args):
    ...
    state = env.reset()
    # state_tensor = torch.from_numpy(np.ascontiguousarray(state)).unsqueeze(0).cuda().to(torch.float32)
    # state = encoder.encoder(state_tensor).squeeze(0).detach().cpu().numpy()
    dtype = state.dtype
    ...
    if our_alg:
        ...
        dtype = state.dtype
        state_tensor = torch.from_numpy(np.ascontiguousarray(state)).unsqueeze(0).cuda().to(torch.float32)
        #state = encoder.encoder(state_tensor).detach().cpu().numpy()
        ...
        #load_path = 'PongNoFrameskip-v4_very_good/att_frame_370000.pth'
        #load_path = 'PongNoFrameskip-v4_mean/att_frame_380000.pth'
        load_path = 'pretrained/Pong-natural.model'
        #load_path = 'sa-dqn_models/models/Pong-natural.model'
        #load_path = "PongNoFrameskip-v4_ran_good/att_frame_270000.pth"
        #load_path = 'FreewayNoFrameskip-v4_tmp/att_frame_2000000.pth'
        #load_path = 'BankHeistNoFrameskip-v4_ori/frame_6000000.pth'
        #load_path = ""
        ...
        #optimizer = optim.Adam([{'params':current_model.features.advantage.parameters()}, {'params':current_model.features.value.parameters()}], lr = lr, eps=training_config['adam_eps'])
        ...
        training_config['use_async_env'] = True
        ...
        #state_diff = diffusion.p_sample_loop_with_prior((state.transpose(0,2,1))/255, steps = 10)[0].cpu().numpy().transpose(0,2,1)
        reload = True
        for frame_idx in range(1, num_frames + 1):
            ...
            if adv_train and eps != np.nan and eps >= np.finfo(np.float32).tiny:
                ...
                #with torch.no_grad():
                state_tensor = torch.from_numpy(np.ascontiguousarray(state)).unsqueeze(0).cuda().to(torch.float32)
                #att_state = get_att_state(current_model, state/255, max_diff_norm = 1/255, sample_time = 10)
                att_state_tensor = pgd(current_model, state_tensor/255, current_model.act(state_tensor/255), env_id = "Pong")
                ...
                #state_diff = diffusion.p_sample_loop_with_prior((state.transpose(0,2,1))/255, steps = 10)[0].cpu().numpy().transpose(0,2,1)
                #att_state_diff = diffusion.p_sample_loop_with_prior((att_state.transpose(0,2,1)), samples = 2, steps = 10).cpu().numpy().transpose(0,1,3,2)
                # for _ in range(1):
                #     att_state = np.clip((np.random.uniform(-0.1/255, 0.1/255, size = (1,84,84)) + att_state), 0 ,1)
                for _ in range(1):
                    att_state = np.clip((np.random.normal(0, 1/255, size = (1,84,84)) + att_state), 0 ,1)
                att_state_diff = make_visualization_withprior(teacher_diffusion, device, [2, 1, 84, 84], att_state.transpose(0,2,1), need_tqdm=False, eta=0, clip_value=1.2).cpu().numpy().transpose(0,1,3,2)

                cv2.imwrite('testpic_diff.png', att_state_diff[0].transpose(2,1,0)*255)
                #cv2.imwrite('testpic_ori.png', state.transpose(2,1,0))
                #print(att_state_diff.shape)
                #state_diff, att_state_diff = diffusion.p_sample_loop_with_prior_combine((state.transpose(0,2,1))/255, (att_state.transpose(0,2,1)), samples = 2, steps = 10)
                #state_diff = state_diff.cpu().numpy().transpose(0,2,1)
                #att_state_diff = att_state_diff.cpu().numpy().transpose(0,1,3,2)

                #print(att_state_diff.shape)
                #att_state_diff = att_state_diff.transpose(0,2,1)
                #att_state_tensor = torch.from_numpy(np.ascontiguousarray(att_state_diff)).unsqueeze(0).cuda().to(torch.float32)
                # if len(belief)==0:
                #     action = current_model.act(att_state_tensor, act_epsilon)[0]
                # else:
                #     eps = linear_schedule_exp(0.001, 0.5, 500000, frame_idx)
                #     action = get_action_withbelief_atari(env, current_model, encoder, belief, eps)
                    #print(frame_idx, action)

                if buffer_stored_size > replay_initial:
                    eps = linear_schedule_exp(0.01, 0.05, 100000, frame_idx)
                else:
                    eps = 0
                action = get_action_withbelief(env, current_model, att_state_diff, eps)
                #print(action)

                #record history trajectory

                # obs.append(encoder.encoder(current_model.features.cnn(att_state_tensor)).detach().cpu().numpy())
                # #temp_state = copy.deepcopy(state)
                # #temp_state = np.asarray(temp_state)
                # pos.append(encoder.encoder(current_model.features.cnn(ori_state_tensor)).detach().cpu().numpy())
                # actions.append([action])
                # obs_tensor = torch.Tensor(np.asarray(obs)).unsqueeze(0).cuda()
                # pos_tensor = torch.Tensor(np.asarray(pos)).unsqueeze(0).cuda()
                # actions_tensor = torch.Tensor(np.asarray(actions)).unsqueeze(1).cuda()
                #
            ...
            if training_config['use_async_env']:
                ...
                next_state_tensor = torch.from_numpy(np.ascontiguousarray(state)).unsqueeze(0).cuda().to(torch.float32)
                next_att_state_tensor = pgd(current_model, next_state_tensor/255, current_model.act(next_state_tensor/255), env_id = "Pong")
                next_att_state = next_att_state_tensor.cpu().numpy()[0]
                for _ in range(1):
                    next_att_state = np.clip((np.random.normal(0, 1/255, size = (1,84,84)) + next_att_state), 0 ,1)
                next_state_diff = make_visualization_withprior(teacher_diffusion, device, [1, 1, 84, 84], next_att_state.transpose(0,2,1), need_tqdm=False, eta=0, clip_value=1.2)[0].cpu().numpy().transpose(0,2,1)
            log_time('env_time', time.time() - t)
            ...
            if training_config['use_async_env']:
                ...
                if use_cpp_buffer:
                    #replay_buffer.add(obs=state, act=action, rew=reward, next_obs=next_state, belief = belief, done=done)
                    #print(replay_buffer)
                    replay_buffer.add(obs=state_diff*255, act=action, rew=reward, next_obs=next_state_diff*255, belief = att_state_diff, done=done)
                else:
                    replay_buffer.push(state, action, reward, next_state, done)
                log_time('save_time', time.time() - t)
            ...
            if frame_idx % training_config['save_frame'] == 0 or frame_idx==num_frames:
                ...
                torch.save(current_model.features.state_dict(), '{}/att_frame_{}.pth'.format(prefix, frame_idx))
            ...
            if frame_idx % 5000 == 0:
                print('frame_index', frame_idx, 'total', time.time() - frame_start)
