...
def mini_test(env, model, diffusion, config, dtype, max_frames_per_episode=1000, seed = 1000):
    ...
    old_steps, pa_attacker_state, _ = \
            torch.load('sa-dqn_models/paad/dqn_obspol_attacker_FreewayNoFrameskip-v4',  map_location=device)
    ...
    for frame_idx in range(1, max_frames_per_episode + 1):
        ...
        #print(model.act(state_tensor, 0))
        # att_state_tensor = pgd(model, state_tensor, model.act(state_tensor, 0), env_id = "Pong")
...
def main(args):
    ...
    model_path = "sa-dqn_models/radial_models/Freeway-15-ra-final.pt"
    ...
    file = open("minbest-15-255-Freeway-radial.csv", 'w')
    writer = csv.writer(file)
    writer.writerow(reward)
    file.close()
    file = open("Freeway-minbest-radial-fps.csv", 'w')
    writer = csv.writer(file)
    writer.writerow(fps)
    file.close()
