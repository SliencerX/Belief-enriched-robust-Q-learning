...
def main(args):
    ...
    if our_alg:
        ...
        diffusion.load_state_dict(torch.load("pretrained/Diffusion-pong.pt")['model'])
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            if adv_train and eps != np.nan and eps >= np.finfo(np.float32).tiny:
                ...
                eps = linear_schedule_exp(0.01, 0.2, 200000, frame_idx)
            ...
            if training_config['use_async_env']:
                ...
                next_state_diff = diffusion.p_sample_loop_with_prior((next_state.transpose(0,2,1)/255), steps = 10)[0].cpu().numpy().transpose(0,2,1)
            log_time('env_time', time.time() - t)
