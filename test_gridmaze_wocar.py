...
def mini_test(model, config, dtype, num_episodes=10, max_frames_per_episode=100):
    ...
    for frame_idx in range(1, num_episodes * max_frames_per_episode + 1):
        ...
        #
        # att_state_tensor = min_best(model, state_tensor, 0.1, pgd_steps=10, lr=1e-2, fgsm=False,
        ...
        if done:
            ...
            this_episode_frame += 1
            frame_count+=1
    print(frame_idx/(time.time()-begin_time_1))
...
def main(args):
    ...
    #model_path = "sa-dqn_models/wocar_models/Grid_continous_wocar.pth"
    model_path = "sa-dqn_models/wocar_models/Grid_continous_wocar_1.pth"
    ...
    # file = open("vanila-gridmaze-wocar.csv", 'w')
    # writer = csv.writer(file)
    # writer.writerow(reward)
    # file.close()
    # print("average reward is ", np.mean(reward))

    file = open("vanilla-gridmaze-wocar-fps.csv", 'w')
    writer = csv.writer(file)
    writer.writerow(fps)
    file.close()
    print("average reward is ", np.mean(reward))
